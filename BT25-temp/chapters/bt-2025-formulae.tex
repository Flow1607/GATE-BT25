...
\begin{itemize}
\item For a first-order differential equation $\frac{dy}{dt} = f\brak{t, y}$ with step size $h$[cite: 6]:
\begin{align}
...
For a real-valued differentiable function $f\brak{x}$, the root is iteratively approximated using its first-order Taylor series expansion about the current estimate $x_n$[cite: 31]:
\begin{align}
